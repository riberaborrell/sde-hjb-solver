   ...
   '''
    ...
    def solve_bvp_det(self):
        ...
        for l in range(self.K - 1, -1, -1):
            ...
            # assemble linear system of equations: \psi^{l-1} = A \psi^l + b
            ...
            A_left_inv = np.linalg.pinv(A[:, 1:29])
            self.psi[l, 1:29] = np.dot(A_left_inv, self.psi[l+1, :])
            self.psi[l, idx_ts] = b[idx_ts]
